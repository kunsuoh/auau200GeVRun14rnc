                               ...
                               StPxlHitCollection& pxlHitCol)
{
   ...
   for (UInt_t iSec = 0; iSec < mcPxlHitCol.numberOfSectors(); iSec++)
   {
      ...
      for (UInt_t iLad = 0; iLad < mcPxlSectorHitCol->numberOfLadders(); iLad++)
      {
		...
		localToMatser(localPixHitPos,globalPixHitPos,iSec+1,iLad+1,iSen+1);
                //cout << "key() : " << mcPix->key() - 1 << " idTruth: " << mcPix->parentTrack()->key() << endl;
		cout << "glabalPixlHitPos = " << globalPixHitPos[0] << " " << globalPixHitPos[1]  << " " << globalPixHitPos[2] << endl;
               ...
               //cout << "smearedlocal = " << localPixHitPos[0] << " " << localPixHitPos[1] << " " << localPixHitPos[2] << endl;
	       ...
	       localToMatser(localPixHitPos,smearedGlobalPixHitPos,iSec+1,iLad+1,iSen+1);
               ...
               //cout << "key() : " << mcPix->key() - 1 << " idTruth: " << mcPix->parentTrack()->key() << endl;
               //cout << "from StMcPxlHit : x= " << mcPix->position().x() << ";  y= " << mcPix->position().y() << ";  z= " << mcPix->position().z() << endl;
               cout << "pxlHit location x= " << tempHit->position().x() << "; y= " << tempHit->position().y() << "; z= " << tempHit->position().z() << endl;
